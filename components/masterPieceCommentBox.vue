<template>
  <div>
    ...
    <form v-on:submit.prevent>
      <input type="text" v-model="commentContent" placeholder="随便说点什么吧~" class="reply-text" />
      <div class="reply-btn-show">
        <i class="icon-happy iconfont icon-smile"></i>
        <button :disabled="canSubmit" @click="submitComment" class="Reactor-building-reply-btn">评论</button>
      </div>
    </form>
        ...
        <div class="review-layer">
          <div class="review-layer-img">
            <a href="#" class="review-layer-img-show">
              <progressive-img :src="item.user.avatar" placeholder="../img/loading.gif" />
            </a>
    ...
    <div v-if="comments.total>=32" class="article-page">
      <pagination :total="comments.total" :page-size="comments.per_page" :callback="pageChanged" :options="paginationOptions" nav-class="padding-10" ul-class="floor-answer-Inputbox" li-class="txt-color-blue">
      </pagination>
    </div>
  </div>
</template>
